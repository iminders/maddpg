...
class GaussianActor(tf.keras.Model):
    ...
    def __init__(self, state_shape, action_dim, max_action, units=(256, 256),
        ...
        self.base_layers = []
        for unit in units:
            self.base_layers.append(layers.Dense(
                unit, activation=hidden_activation))
    ...
    def _compute_dist(self, states):
        ...
        if self._state_independent_std:
            ...
            log_std = tf.clip_by_value(
                log_std, self.LOG_STD_CAP_MIN, self.LOG_STD_CAP_MAX)
    ...
    def call(self, states, test=False):
        ...
        if self._squash:
            ...
            diff = tf.reduce_sum(tf.math.log(
                1 - actions ** 2 + self.EPS), axis=1)
            log_pis -= diff
        else:
            actions = raw_actions
